...
@section('pageHeaderTwo', '团队长业绩分红奖')
@section('pageHeaderTwoUrl', '/admin/performance_bonus/index')
...
@section('body')
    ...
    <script type="text/html" id="toolbarDemo">
        <div class="layui-btn-container">
            @if(Gate::forUser(auth('admin')->user())->check('admin.performance_bonus.create'))<button class="layui-btn layui-btn-sm" lay-event="create">新增</button>@endif
        </div>
    </script>
@stop
    ...
    <script>
        layui.use(['table', 'form'], function () {
            ...
            table.render({
                elem: '#demo'
                , toolbar: '#toolbarDemo'
                , url: '/admin/performance_bonus/index'
                , method: 'POST'
                , where: {
                    "_token":"{{csrf_token()}}"
                }
            ...
            table.on('tool(test)', function (obj) {
                ...
                if (obj.event === 'del') {
                    layer.confirm('真的拒绝么', function (index) {
                        $.ajax({
                            url: "/admin/performance_bonus/ajax",
                            type: "POST",
                            data: {
                                '_token': "{{ csrf_token() }}",
                                'id': data.id,
                    ...
                    layer.confirm('真的删除行么', function (index) {
                        $.ajax({
                            url: "/admin/performance_bonus/" + data.id,
                            type: "POST",
                            data: {
                                '_method': "DELETE",
                                '_token': "{{ csrf_token() }}"
                    ...
                    location.href = '/admin/performance_bonus/' + data.id + '/edit';
                }
            });
            ...
            table.on('toolbar(test)', function (obj) {
                switch (obj.event) {
                    case 'create':
                        location.href = '/admin/performance_bonus/create';
                        break;
                }
            });
